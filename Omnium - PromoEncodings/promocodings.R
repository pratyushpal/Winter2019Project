...
# Requires the time period to be in ascending order
promo_code_testing <- function(avg_price, promo, time_period){
  time_period <- order(time_period, decreasing = FALSE)
  avg_price <- price_to_num(avg_price)
  plot(x=time_period, y=avg_price,type='l', ylab = "Average Price", xlab = 'Time Period', col="red")
  par(new=TRUE)
  plot(x=time_period , y=promo,ylab = "", xlab = "", col="blue")
}
